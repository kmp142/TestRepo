...
def detect_neighbors(points, point_index, eps):
    neighbors = []
    for i, point in enumerate(points):
        if calculate_distance(points[point_index], point) < eps:
            neighbors.append(i)
    return neighbors
...
def dbscan_algorithm(points, eps, min_samples):
    ...
    for i, point in enumerate(points):
        if visited[i]:
            continue
        visited[i] = True
        neighbors = detect_neighbors(points, i, eps)
...
def expand_cluster(points, cluster_assignment, visited, point_index, neighbors, cluster_id, eps, min_samples):
    for neighbor_index in neighbors:
        if not visited[neighbor_index]:
            ...
            new_neighbors = detect_neighbors(points, neighbor_index, eps)
